 ...
 * are operational data, not code — they belong in Netlify's environment so the
...
export function profile() {
  ...
  const raw = process.env.ES_PROFILE;
  if (!raw) {
  ...
  try {
    ...
    throw new Error(
      `ES_PROFILE is not valid JSON (${error.message}). Check the value in the Netlify environment variables.`,
    );
  }
  return parsed;
}
