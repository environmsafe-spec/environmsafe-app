 ...
 * company's filing system. See netlify/lib/config.js and AGENT_SETUP.md.
 *
...
export function buildSystemPrompt() {
  const { company: co, folders, customers, suppliers } = profile();

  const folderList = FOLDER_NAMES.filter((name) => folders[name])
    .map((name) => `  - "${name}" → ${folders[name]}`)
    .join("\n");
...
All procurement documents live in Google Drive under a numbered folder system. Use these folder IDs directly — do not go hunting for them:

${folderList || "  (no folders configured — set ES_PROFILE.folders)"}
